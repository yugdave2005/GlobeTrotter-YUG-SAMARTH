      ...
      <nav className="flex-1 px-6 py-8 space-y-2 overflow-y-auto">
        {navItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center px-4 py-3 rounded-2xl font-medium transition-all duration-200 relative ${
                isActive
            ...
            {({ isActive }) => (
              <>
                ...
                {isActive && (
                  <motion.div
                    layoutId="activeNav"
                    className="absolute inset-0 bg-primary-50 rounded-2xl z-0 border border-primary-100"
                    initial={false}
